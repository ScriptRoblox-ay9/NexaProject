...
local packet_io_t = (function()
	...
	local function pair_read(bs, pair, depth)
		depth = depth or 0
		if depth > 50 then 
			error("Max depth exceeded (possible cyclic reference in packet structure)")
		end
		...
		if pair[1].read then
			...
			for _, jpair in ipairs(pair[1]) do
				t[(jpair[2])] = pair_read(bs, jpair, depth + 1)
			end
			return t
		end
	end
	...
	function this:read(bs, packet)
		if self.sizeof == 0 then
			...
			for _, pair in ipairs(self.struct) do
				packet[(pair[2])] = pair_read(bs, pair, 0)
			end
		end
	end
...
local function create_packet_handler(id, first_field, default_event)
	...
	function this.read(bs)
		...
		if not status then
			return default_event, { { id = packet.id, bytes = {} } }
		end
	...
	function this.write(bs, data)
		...
		pcall(function() packet_io:write(bs, packet) end)
	end
...
function module.decode(packet, decoder)
	...
	if not packet.text then return false end
	local event, json = packet.text:match("^window.executeEvent%('(.-)', .(.+).%);$")
	if not event or not json then return false end
...
local function process_packet(id, bs, handlers)
	...
	if handler then
		...
		local success, event, data = pcall(handler.read, bs)
		if not success or not event or not data then 
			return
		end
		...
		if module[event] then
			...
			elseif type(result) == "table" then
				if id == 220 and result[1] and result[1].id == 17 then
					subprocess.push(id, result, handler.write)
				else
					BitStream.setWriteOffset(bs, 8)
					pcall(handler.write, bs, result)
				end
			end
		end
	end
